...
class BienImmobilierController extends AbstractController
{
    ...
    public function update(Request $request, BienImmobilierRepository $bienImmobilierRepository,EntityManagerInterface $entityManagerInterface, BienImmobilier $BienImmobilier): Response
    {
        $form =$this->createForm(BienImmobilierType::class, $BienImmobilier);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
