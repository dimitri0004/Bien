...
class PropreitaireController extends AbstractController
{
    ...
    #[Route('/propreitaire', name: 'app_propreitaire')]
    public function update(Request $request, PropreitaireRepository $propreitaireRepository, EntityManagerInterface $entityManagerInterface, Propreitaire $propreitaire): Response
    {
        $form =$this->createForm(PropreitaireType::class, $propreitaire);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
